...
class UserController extends Controller
{
    ...
    public function pendingActivities(Request $request)
    {
        ...
        $status = $request->status ? $request->status : null;
        $search = $request->search ? $request->search : null;
        $notes = Activitie::where('status','Pending')->orderBy('id', 'DESC');
    ...
    public function activities_list(Request $request)
    {
        ...
        $status = $request->status ? $request->status : null;
        $search = $request->search ? $request->search : null;
        $notes = Activitie::whereIn('status',['Approved','Rejected'])->orderBy('id', 'DESC');
            ...
            if($validation->fails()) {
              ...
              if (!$invest_check)
              {
                   ...
                   else
                 {
                  ...
                  sendEmail($user->email, 'Account Activated -'.siteName(), [
                    'name' => $user->name,
