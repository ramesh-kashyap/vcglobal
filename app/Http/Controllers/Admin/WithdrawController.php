...
class WithdrawController extends Controller
{
    ...
    public function withdrawalInfo(Request $request)
    {
        ...
        $status = $request->status ? $request->status : null;
        $search = $request->search ? $request->search : null;
        $notes = Walletwithdraw::orderBy('id','DESC');
        ...
        $notes = $notes->where(function($q) use($search){
            ...
            ->orWhere('withdraw_method', 'LIKE', '%' . $search . '%')
        ...
        $this->data['withdraw_list'] =  $notes;
        $this->data['search'] = $search;
        $this->data['page'] = 'admin.withdraw.withdraw-info';
        return $this->admin_dashboard();
    }
  ...
  public function pendingBankWithdrawal(Request $request)
  {
        ...
        $f_bal=calFundingBalance($user_id->id);
        $updated_balance = $f_bal + $withdraw_id->amount;
        $user_id->update([
