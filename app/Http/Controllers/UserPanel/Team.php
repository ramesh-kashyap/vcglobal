...
// $user=Auth::user();
$this->data['page'] = 'user.team.refer';
...
// $user=Auth::user();
$this->data['page'] = 'user.team.levels';
return $this->dashboard_layout();

}

public function referprofit(Request $request)
{
    $user = Auth::user();
      ...
      $notes = Income::where('user_id',$user->id)->whereIn('remarks',['Referral Income','Direct Referral Income'])->orderBy('id', 'DESC');
...
//   dd($notes);
    $this->data['referalprofits'] = $notes;
    $this->data['page'] = 'user.team.referprofit';
    return $this->dashboard_layout();
}
        ...
        $notes = $notes->where(function($q) use($search){
          ...
          ->orWhere('active_status', 'LIKE', '%' . $search . '%');
            ...
            $notes = $notes->paginate($limit)
                ...
                $earning=Income::where('user_id',$user->id)->where('remarks','Referral Income')->sum('comm');
               $sponsors=User::where('sponsor',$user->id)->count();
...
public function affiliate_level()

{
$this->data['page'] = 'user.team.affiliate_level';
